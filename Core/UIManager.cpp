...
//#endif
	...
	case WM_PAINT:
		{
				...
				if( m_bUpdateNeeded )
				{
					...
					if( !::IsRectEmpty(&rcClient) )
					{
						...
						else
						{
							...
							while( pControl = m_pRoot->FindControl(__FindControlFromUpdate, NULL, 
								UIFIND_VISIBLE | UIFIND_ME_FIRST) )
							{
			...
			if( m_bUpdateNeeded )
			{
				...
				if( !::IsRectEmpty(&rcClient) )
				{
					if( m_pRoot->IsUpdateNeeded() )
					{
						if( !::IsIconic(m_hWndPaint))		// relativepos属性导致控件错误的bug修复 Fixed by redrain  详情:http://blog.csdn.net/zhuhongshu/article/details/40622169
							m_pRoot->SetPos(rcClient);
						if( m_hDcOffscreen != NULL ) ::DeleteDC(m_hDcOffscreen);
						if( m_hbmpOffscreen != NULL ) ::DeleteObject(m_hbmpOffscreen); 
						m_hDcOffscreen = NULL; 
    ...
    case WM_PRINTCLIENT:
        {
			// 无论是不是透明，这个消息其实都没有响应，就是说这个处理函数其实没用。
			DUI__Trace(_T("WM_PRINTCLIENT wParam : 0x%08X, lParam : 0x%08X"), wParam, lParam);
            RECT rcClient;
            ::GetClientRect(m_hWndPaint, &rcClient);
            HDC hDC = (HDC) wParam;
            int save = ::SaveDC(hDC);
		...
		for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) 
		{
			...
			while(hTempParent)
			{
				if(pMsg->hwnd == pT->GetPaintWindow() || hTempParent == pT->GetPaintWindow())
				{
					...
					// 修正CEditUI和CWebBrowserUI中按Tab键无法切换焦点的bug。 Fixed by Redrain. 详情http://blog.csdn.net/zhuhongshu/article/details/41556615
