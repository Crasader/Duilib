...
// 第一种方法： 不添加stb_image.c到工程，直接include，此方法不推荐  (以下可以不用namespace) [11/3/2014 ybt]
// namespace stb_image {
// 	#include "../Utils/stb_image.c"
// }
// using namespace stb_image;

// 第二种方法：添加stb_image.c到工程，使用extern关键字。但不能使用stb_image.c中定义的结构体
extern "C"
{
...
{
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	if (dwPenColor % 0x01000000 == 0) { dwPenColor += 1; }			// 防止透明模式下，纯黑文字失真 [10/30/2014 ybt] 
	LOGPEN lg;
...
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	if (dwPenColor % 0x01000000 == 0) { dwPenColor += 1; }
...
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	if (dwTextColor % 0x01000000 == 0) { dwTextColor += 1; }
