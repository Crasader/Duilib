...
namespace DuiLib
{
	...
	bool CTreeNodeUI::AddAt( CControlUI* pControl, int iIndex )
	{
		...
		iIndex = CLAMP(iIndex, 0, GetCountChild());		// 不能是GetCountChild() - 1 [11/28/2014 ybt]
	...
	long CTreeViewUI::AddAt( CTreeNodeUI* pControl, int iIndex )
	{
		...
		// 检查iIndex的值 [11/21/2014 ybt]
		iIndex = CLAMP(iIndex, 0, GetCount());			// 不能是GetCount()-1
// 		CTreeNodeUI* pParent = static_cast<CTreeNodeUI*>(GetItemAt(iIndex - 1));		// fixed by Redrain.  http://blog.csdn.net/zhuhongshu/article/details/38145557
// 		if(!pParent)
// 			return -1;
