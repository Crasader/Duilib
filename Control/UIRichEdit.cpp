...
BOOL CTxtWinHost::CanPaste(UINT nFormat)
{
	if (NULL == pserv)
		return FALSE;
	HRESULT lRes = 0;
...
void CTxtWinHost::PasteSpecial(UINT uClipFormat, DWORD dwAspect, HMETAFILE hMF)
{
	if (NULL == pserv)
		return;
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    ...
    if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
        ...
        if(pHover != this) {
			...
			// 修正在RichEditUI上弹出菜单CMenuUI，菜单无法响应消息的Bug [11/27/2014 ybt]
			bWasHandled = false;
		}
    }
    else
