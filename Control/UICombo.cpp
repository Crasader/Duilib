...
{
//	if( m_pWindow != NULL ) m_pWindow->Close();				// 当鼠标滚动时下拉列表自动关闭的bug的修复 by Redrain [11/4/2014 ybt]  
...
void CComboUI::SelectText(LPCTSTR pstrText)
{
	CControlUI* pControl;
	for (int i = 0; i < m_items.GetSize(); i++)
	{
		// 如果CComboUI所在的Layout是隐藏的，那m_items中的每个控件的m_bInternVisible都为false,
		// 如此m pControl->IsVisible()总是返回false，所以SelectItem后总是不能刷新。
		// 暂时的解决办法是在CComboUI所在的Layout在显示状态时调用本函数。 [11/29/2014 ybt]	
		pControl = static_cast<CControlUI*>(m_items[i]);
		if (_tcscmp(pControl->GetText(), pstrText) == 0) {
			SelectItem(i);
			break;
		}
	}
