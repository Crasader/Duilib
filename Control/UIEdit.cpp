...
namespace DuiLib
{
	...
	void CEditWnd::Init(CEditUI* pOwner)
	{
		...
		// WS_POPUP属至关重要，在使用UpdateLayeredWindow处理窗口透明时，子窗口没有WS_POPUP则绘制不出来。
		// 如果不使用UpdateLayeredWindow，没有WS_POPUP仍然可以绘制出来。[8/7/2014 ybt]
		// 但没有WS_CHILD属性的子窗体不能切换Tab键
		UINT uStyle = 0;
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_dwTipTextColor(0xFFC0C0C0), m_iWindowStyls(0), m_bTrans(false)
	{
	...
	void CEditUI::SetTipText(LPCTSTR pstrTipText)
	{
		m_sTipText = pstrTipText;
	}
