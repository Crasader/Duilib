...
namespace DuiLib
{
	...
	SIZE CLabelUI::EstimateSize(SIZE szAvailable)
	{
		...
		// 如果没有指定控件宽度或高度，此做法还有待验证 [11/21/2014 ybt]
		if (m_cxyFixed.cx == 0 || m_cxyFixed.cx == 0)
		{
			RECT rcText = { 0, 0, max(szAvailable.cx, m_cxyFixed.cx), max(szAvailable.cy, m_cxyFixed.cy)};
			...
			if( m_bShowHtml ) {   
				...
				CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, m_dwTextColor, 
					NULL, NULL, nLinks, DT_CALCRECT | m_uTextStyle);
			}
			else {
				CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, m_dwTextColor, 
					m_iFont, DT_CALCRECT | m_uTextStyle);
			}

			// 设置高度宽度
			cXY.cx = rcText.right - rcText.left + m_rcTextPadding.left + m_rcTextPadding.right + 10;
			cXY.cy = rcText.bottom - rcText.top + m_rcTextPadding.top + m_rcTextPadding.bottom;
		}
